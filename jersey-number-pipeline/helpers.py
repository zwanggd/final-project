...
def generate_crops(json_file, crops_destination_dir, legible_results, all_legible = None):
    ...
    for entry in tqdm(all_poses):
        ...
        # Generate the new crop filename according to the new naming convention
        relative_path = os.path.relpath(img_name, '/Users/kai/GSR/data/SoccerNetGS/converted/valid')
        dir_parts = relative_path.split('/')
        if len(dir_parts) >= 3:
            parent_dir = dir_parts[-2]  # This corresponds to '1' in 'SNGS-021/1/0.jpg'
            file_name = os.path.splitext(dir_parts[-1])[0]  # Extract '0' from '0.jpg'
            name = f"{parent_dir}_{file_name}.jpg"
        else:
