...
from models import predict_with_majority_voting, final_prediction, load_finetuned_models, TrackletDataset, DataLoader
import torch
from mmocr.mmocr.datasets import transforms
...
def soccer_net_pipeline(args):
    ...
    if args.pipeline['feat']:
        ...
        print(f"conda run -n {config.reid_env} python {config.reid_script} --tracklets_folder {image_dir} --output_folder {features_dir}")
    ...
    if args.pipeline['filter'] and success:
        ...
        command = f"python /Users/kai/GSR/soccernet/jersey-number-pipeline/gaussian_outliers.py --tracklets_folder {image_dir} --output_folder {features_dir}"
        success = os.system(command) == 0
    ...
    if args.pipeline['pose'] and success:
        ...
        if success:
            ...
            command = f"conda run -n {config.pose_env} python /Users/kai/GSR/soccernet/jersey-number-pipeline/pose.py {config.pose_home}/configs/body/2d_kpt_sview_rgb_img/topdown_heatmap/coco/ViTPose_huge_coco_256x192.py \
    ...
    if args.pipeline['str'] and success:
        ...
        command = f"conda run -n {config.str_env} python /Users/kai/GSR/soccernet/jersey-number-pipeline/str.py  {config.dataset['SoccerNet']['str_model']}\
    ...
    if args.pipeline['combine'] and success:
        ...
        models = load_finetuned_models(features_dir, device)
        ...
        tracklet_dataset = TrackletDataset(image_dir, transform=transform)
    ...
    if(config.dataset['SoccerNet'][args.part]['gt'] != ''):
